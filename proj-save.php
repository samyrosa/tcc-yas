...
$cod = $_SESSION['id'] ?? 0;
$Scod = $_GET['Scod'] ?? 0;
$busca = $banco->query("SELECT * from salvo_proj WHERE  proj_id='$Scod' and user_id='$cod'");
...
if (!is_logado()) {
  ...
  if ($busca->num_rows > 0) {
    if ($banco->query("delete from salvo_proj WHERE  proj_id='$Scod' and user_id='$cod'")) {

      header("Location: proj-view.php?cod=$Scod");
    } else {
      echo msg_erro('Opp..', 'Falha na busca do banco de dados, por favor tente denovo', 'Tentar Novamente', 'index.php');
    }
  } else {
    if ($banco->query("INSERT INTO salvo_proj (proj_id, user_id) VALUES ($Scod,$cod)")) {
